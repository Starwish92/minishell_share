...
typedef struct s_cmd //s_cmd_info
{
	...
	struct s_cmd	*prev; //prev
	struct s_cmd	*next; //next
}						t_cmd; //t_cmd_info

typedef struct s_env //s_info_env
{
	char			*key; //env_key
	char			*value; //env_val
	struct s_env	*next; //next
	struct s_env	*prev; //prev
}	t_env; //t_info_env
